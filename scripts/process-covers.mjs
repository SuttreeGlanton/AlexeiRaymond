...
const OUT_COVERS_DIR = path.join(ROOT, 'public', 'covers');
...
async function run() {
  ...
  for (const file of pieceFiles) {
    ...
    if (fileSlug === INTERVIEW_SLUG) {
      const outName = 'interview.jpg';
      await processOne(path.join(RAW_PIECES_DIR, file), path.join(OUT_COVERS_DIR, outName), {
        width: 1500,
        height: 1800
      });
      data.interview.cover = `covers/${outName}`;
      report.interview.push(`${file}  ->  covers/${outName}`);
      continue;
    }
  ...
  const stillMissing = [
    ...
    ...(data.interview.cover ? [] : ['interview'])
  ];
  ...
  if (report.unmatchedFiles.length > 0) {
    ...
    console.log('Rename to match a cycle id/name, an exact piece title, or "interview", then re-run.');
  }
